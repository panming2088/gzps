...
public class FeedbackLayerService extends AgwebPatrolLayerService2{
    ...
    public FeedbackLayerService(Context mContext) {
       super(mContext);
    }
    ...
    public void queryUploadDataInfo(Point point, SpatialReference spatialReference, double resolution,
        ...
        if ("/0".equals(url)) {
            callback2.onFail(new Exception("无法查询到交办反馈图层的url"));
            return;
        }
...
//
                final List<UploadInfo> uploadedFacilities = new ArrayList<>();
                //判断是新增还是纠错
     ...
     * @param callback2
     */
    public void queryUploadDataForObjectId(final Callback2<List<UploadInfo>> callback2) {
        String url = getFeedbackLayerUrl() + "/0";
