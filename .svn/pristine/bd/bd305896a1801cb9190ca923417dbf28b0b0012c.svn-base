...
public class UploadFeedbackTableViewManager {
    ...
    public UploadFeedbackTableViewManager(
            Context context,
...
//            }
            ...
            if(!StringUtil.isEmpty(uploadedFacility.getOLDOID())){
                tableitem_check_id.setText(uploadedFacility.getOLDOID());
            }
            addAttributeView();
    ...
    private void initView() {
        ...
        root = LayoutInflater.from(mContext).inflate(R.layout.view_upload_feedback_facility_table_view, null);
        tableitem_checkstate = (TextItemTableItem) root.findViewById(R.id.tableitem_checkstate); //审核状态
        tableitem_check_id = (TextItemTableItem) root.findViewById(R.id.tableitem_check_id); //核查编号
