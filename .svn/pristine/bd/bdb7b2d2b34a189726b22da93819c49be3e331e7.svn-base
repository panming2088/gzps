...
import com.augurit.agmobile.gzps.journal.view.reeditjournal.ReEditModifiedJournalActivity;
...
public class DayPatrolDetailFragment extends Fragment implements View.OnClickListener {
    ...
    private JournalService deleteFacilityService;
    ...
    public static DayPatrolDetailFragment getInstance(Bundle data) {
        DayPatrolDetailFragment addComponentFragment2 = new DayPatrolDetailFragment();
...
//                    }else
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        if (mCurrentModifiedFacility != null) {
                            if("排水口".equals(mCurrentModifiedFacility.getLayerName())){
                                Intent intent = new Intent(mContext, AddNewPfkJournalActivity.class);
                                intent.putExtra("data", mCurrentModifiedFacility);
                                startActivity(intent);
                            }else{
                                Intent intent = new Intent(mContext, ReEditModifiedJournalActivity.class);
                                intent.putExtra("data", mCurrentModifiedFacility);
                                startActivity(intent);
                            }
    ...
    private void initDeleteFacilityService() {
        if (deleteFacilityService == null) {
            deleteFacilityService = new JournalService(mContext);
        }
    }
...
//            final ProgressDialog progressDialog = new ProgressDialog(mContext);
            final ProgressDialog progressDialog = new ProgressDialog(getActivity());//context有时为空
            progressDialog.setMessage("正在更新设施信息");
            progressDialog.show();

            JournalService correctFacilityService = new JournalService(mContext);
            correctFacilityService.getModifiedDetail(modifiedFacility.getId())
