 ...
 * @version 1.0
 */
public class DrawQustionViewTool implements OnSingleTapListener {
    ...
    private OnQuestionSureListener mSureListener;

    public DrawQustionViewTool(MapView map) {
...
//        this.mLineSymbol = new SimpleLineSymbol(-16777216, 3.0F);
    ...
    public void setSureListener(OnQuestionSureListener sureListener) {
    ...
    public void stopDraw() {
        ...
        isFinished = true;
        mMeasureMode = MeasureType.AREA;  // 模式还原为测距
    }
    ...
    private void addPoint(float x, float y) {
        ...
        if(mPoints.size() == 3){
            if (mOnMeasureGraphicChangeListner != null){
                mOnMeasureGraphicChangeListner.onBeginButtonShow();
            }
        }
    }
    ...
    public void undo() {
        if (mPoints != null){
            if (mPoints.size() >0){
                ...
                if(mPoints.size()==2){
                    if (mOnMeasureGraphicChangeListner != null){
                        mOnMeasureGraphicChangeListner.onButtonEmpty(); //界面没有Graphic
                    }
                }
            }
        }
            ...
            if(this.mCallout == null) {
                ...
                mText.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if(mSureListener != null && mPolygon != null){
                            mSureListener.onClickQuestionSure(mPolygon);
                        }
                    }
                });
...
//            this.mText.setText(this.getResultString());
    ...
    public interface OnMeasureGraphicChangeListner{
        ...
        void onBeginButtonShow(); //当开始绘制Graphic的时候
        void onButtonEmpty(); //当开始绘制Graphic的时候
    }
