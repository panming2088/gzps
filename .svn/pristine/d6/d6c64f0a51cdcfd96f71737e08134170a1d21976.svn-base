package com.augurit.agmobile.gzps.journal.view.uploadevent;
 ...
 * Rg或Rm上报
 */
public class JournalEventUploadActivity extends BaseActivity {
...
//    private Button btn_start_date;
    ...
    private List<DictionaryItem> mPsk_type_list;
    ...
    private void initData() {
        ...
        dbService = new TableDBService(JournalEventUploadActivity.this);
        facility_type_list = dbService.getDictionaryByTypecodeInDB("A174");
        upload_type_list = dbService.getDictionaryByTypecodeInDB("A178");
        //排水口
        mPsk_type_list = dbService.getChildInVisableDictionaryByPCodeInDB("A174008");
        /*
...
//        initFacilityView(facility_type_list);
    ...
    private void initEventCode(String code) {
        ...
        if (!StringUtil.isEmpty(code)) {
            ...
            for (int j = 0; j < strs.length; j++) {
                //包含其他 手动填写类型
                if (!strs[j].contains("其他")) {
                    for (int i = 0; i < mPsk_type_list.size(); i++) {
                        if (mPsk_type_list.get(i).getCode().equals(strs[j])) {
                            //排水口要getCode才能和下面的默认值的getCode对应上
                            eventCode += "," + mPsk_type_list.get(i).getCode();
                        }
                    }
                } else {
                    et_other.setVisibility(View.VISIBLE);
                    et_other.setText(strs[j].replace("其他：",""));
                }
            }
        }
...
//        btn_start_date = (Button) findViewById(R.id.btn_start_date);
        ...
        progressDialog = new ProgressDialog(JournalEventUploadActivity.this);
    ...
    private void initListener() {
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                       ...
                       /* ||user.getRoleCode().contains(GzpsConstant.roleCodes[3])
                        || user.getRoleCode().contains(GzpsConstant.roleCodes[4]) */)) {
                            //Rg和Rm暂时不能上报
                            ToastUtil.longToast(JournalEventUploadActivity.this,
        ...
        findViewById(R.id.ll_select_component).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (modifiedFacility != null) {
                    Intent intent = new Intent(JournalEventUploadActivity.this, EventDetailMapActivity.class);
                    intent.putExtra("layerName", modifiedFacility.getLayerName());
                    intent.putExtra("layerUrl", modifiedFacility.getLayerUrl());
                    ...
                    intent.putExtra("object_id", modifiedFacility.getObjectId());
                    startActivity(intent);
                } else if (uploadFacility != null) {
                    Intent intent = new Intent(JournalEventUploadActivity.this, EventDetailMapActivity.class);
                    intent.putExtra("layerName", uploadFacility.getLayerName());
                    intent.putExtra("layerUrl", uploadFacility.getLayerUrl());
...
//                    findViewById(R.id.problem_type).setVisibility(View.VISIBLE);
        ...
        findViewById(R.id.ll_back).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DialogUtil.MessageBox(JournalEventUploadActivity.this, "提示", "是否确定放弃本次编辑？", new DialogInterface.OnClickListener() {
                    @Override
                    ...
                    public void onNext(List<OrgItem> list) {
                        if (list != null &&
                            ...
                            for (int i = 0; i < size; i++) {
                                RadioButton radioButton = new RadioButton(JournalEventUploadActivity.this);
                                ...
                                if(modifiedFacility != null && "排水口".equals(modifiedFacility.getLayerName())){
...
//            roadItem.setText("");
        ...
        Observable.create(new Observable.OnSubscribe<HashMap<String, RequestBody>>() {
            ...
            public void call(Subscriber<? super HashMap<String, RequestBody>> subscriber) {
                ...
                .subscribe(new Subscriber<HashMap<String, RequestBody>>() {
                    ...
                    public void onError(Throwable throwable) {
                        progressDialog.dismiss();
                        ToastUtil.shortToast(JournalEventUploadActivity.this, "提交失败，请重试！");
                    }
    ...
    private void sendToServer(HashMap<String, RequestBody> photoBody) {
        UploadEventService eventService = new UploadEventService(JournalEventUploadActivity.this);
                ...
                .subscribe(new Subscriber<Boolean>() {
                    ...
                    public void onNext(Boolean s) {
                        if (!s) {
                            ...
                            progressDialog.dismiss();
                            ToastUtil.shortToast(JournalEventUploadActivity.this, "提交成功");
...
//            problemUploadBean.setLayerurl(mSelComponent.getLayerUrl());
            ...
            if (!StringUtil.isEmpty(modifiedFacility.getLayerUrl())) {
                problemUploadBean.setLayerurl(modifiedFacility.getLayerUrl());
            }
            if (!TextUtils.isEmpty(layerId)) {
                problemUploadBean.setLayer_id(layerId);
            } else {
...
//            problemUploadBean.setLayerurl(mSelComponent.getLayerUrl());
        ...
        problemUploadBean.setSBR(BaseInfoManager.getUserName(JournalEventUploadActivity.this));
    ...
    private ProblemUploadBean getDraftParamsEntity() {
        problemUploadBean.setSfjb("2");  //交办标识符
        diseaseCode = "";
        if ("true".equals(isbyself)) {
            if (!TextUtils.isEmpty(problem_tab_item_self.getText())) {
                problemUploadBean.setSelf_process_detail(problem_tab_item_self.getText());
