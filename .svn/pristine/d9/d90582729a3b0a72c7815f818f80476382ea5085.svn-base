    ...
    private Image getImageByPath(String path) {
        if (mImages != null && mImages.size() > 0) {
            for (Image image : mImages) {
                if (image.getFilePath().equalsIgnoreCase(path)) {
                    return image;
                }
            }
        }
...
//            File imageFile = new File(data.getFilePath());
            if (data.getFilePath() != null && !"".equals(data.getFilePath())) {
                // 显示图片
//                Glide.with(mContext)                             //配置上下文
////                .load(Uri.fromFile(new File(path)))      //设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
//                        .load(data.getFilePath())      //设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
//                        .placeholder(R.drawable.mis_default_error)
//                        .error(R.drawable.mis_default_error)//加载错误之后的错误图
//                        .override(mGridWidth, mGridWidth)//指定图片的尺寸
//                        .centerCrop()
