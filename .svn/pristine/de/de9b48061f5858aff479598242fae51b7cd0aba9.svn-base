...
package com.augurit.am.cmpt.widget.bottomsheet;
...
public class AnchorSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> implements IBottomSheetBehavior{
    ...
    //锚点相对父组件顶端的偏移
    private int anchorOffect;

    private boolean isAllowDragdown = true;
    ...
    private AnchorSheetBehavior.BottomSheetCallback mCallback;
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        ...
        if (!child.isShown() || (!isAllowDragdown && mState == STATE_EXPANDED) || mNestedScrollingChildRef == null) {
            return false;
        }
    ...
    public boolean onTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        ...
        if (!child.isShown() ||  (!isAllowDragdown && mState == STATE_EXPANDED)) {
            return false;
        }
        int action = MotionEventCompat.getActionMasked(event);
     ...
     *              {@link #STATE_HIDDEN}.
     */
    public final void setState(@AnchorSheetBehavior.State int state) {
        if (state == mState) {
            return;
        }
        if (mViewRef == null) {
    ...
    private void setStateInternal(@AnchorSheetBehavior.State int state) {
        if (mState == state) {
            return;
        }
        int oldState = mState;
