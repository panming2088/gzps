...
public class UploadFeedbackActivity extends BaseActivity implements IDrawerController {

    private DrawerLayout drawer_layout;
    private ViewGroup progress_linearlayout;
    private UploadFeedbackMapFragment uploadDistributeMapFragment;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_feedback_facilitymap);
        progress_linearlayout = (ViewGroup) findViewById(R.id.progress_linearlayout);
        drawer_layout = (DrawerLayout) findViewById(R.id.drawer_layout);
        //设置右面的侧滑菜单只能通过编程来打开
        drawer_layout.setDrawerLockMode(DrawerLayout.LOCK_MODE_LOCKED_CLOSED,
                GravityCompat.END);
//        ((TextView)findViewById(R.id.tv_title)).setText("交办反馈");
...
//        });
        ...
        UploadedFacility uploadFacility = getIntent().getParcelableExtra("data");
        Bundle bundle = new Bundle();
        bundle.putParcelable("uploadedFacility",uploadFacility);
        uploadDistributeMapFragment = UploadFeedbackMapFragment.getInstance(bundle);
    ...
    public void onBackPressed(){
        if(!uploadDistributeMapFragment.isHidden()){
            uploadDistributeMapFragment.onBackPressed();
        } else {
            finish();
        }
    }
