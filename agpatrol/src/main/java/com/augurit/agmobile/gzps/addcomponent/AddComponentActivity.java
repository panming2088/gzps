...
public class AddComponentActivity extends BaseActivity {
    ...
    private static final String baseMapUrl = "http://192.168.19.95:6080/arcgis/rest/services/ditu/MapServer";
    private static final String yinjinUrl = "http://192.168.19.95:6080/arcgis/rest/services/testwfs/FeatureServer/0";
    private static final String guanxianUrl = "http://192.168.19.95:6080/arcgis/rest/services/testwfs/FeatureServer/1";
...
//    ArcGISFeatureLayer fl3;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            Fragment dialogFrag = getSupportFragmentManager().findFragmentByTag(TAG_DIALOG_FRAGMENTS);
            if (dialogFrag != null) {
...
//                "http://192.168.19.95:6080/arcgis/rest/services/testwfs/FeatureServer/2",
//                ArcGISFeatureLayer.MODE.ONDEMAND);
//        fl3.setOnStatusChangedListener(statusChangedListener);

        ComponentMap componentMap1 = new ComponentMap();
        componentMap1.setName("窨井");
        componentMap1.setUrl(yinjinUrl);
        componentMap1.setLayer(fl1);
        ComponentMap componentMap2 = new ComponentMap();
        componentMap2.setName("排水管线");
        componentMap2.setUrl(guanxianUrl);
        componentMap2.setLayer(fl2);

        componentMapList.add(componentMap1);
        componentMapList.add(componentMap2);
...
//        mMapView.addLayer(fl3);
        ...
        fl1.setSelectionSymbol(sms);
        fl2.setSelectionSymbol(sls);
//        fl3.setSelectionSymbol(sfs);
        ...
        mMapView.setOnTouchListener(new MyTouchListener(AddComponentActivity.this, mMapView));
...
//                actionSave();
    ...
    private void showAddDialog() {
        listAdapter = new AttributeListAdapter(AddComponentActivity.this, mTemplateLayer.getFields(), mTemplateLayer.getTypes(),
        ...
        AddComponentActivity.this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                AddComponentActivity.this.showDialog(ATTRIBUTE_EDITOR_DIALOG_ID);
            ...
            public void run() {
                if (results != null) {
                    if (results[0][0].isSuccess()) {
                        String msg = AddComponentActivity.this.getString(R.string.saved);
                        Toast.makeText(AddComponentActivity.this, msg, Toast.LENGTH_SHORT).show();
                    } else {
                        ...
                        frag.show(getSupportFragmentManager(), TAG_DIALOG_FRAGMENTS);
                    }
                }
                setProgressBarIndeterminateVisibility(false);
                exitEditMode();
...
//                    }
    ...
    private void addTemplates(ArcGISFeatureLayer featureLayer, FeatureTemplate[] templates) {
        ...
        for (FeatureTemplate featureTemplate : templates) {
            ...
            mFeatureTypeList.add(new FeatureTypeData(bitmap, objectName, symbol));
                ...
                featureLayer.selectFeatures(query, ArcGISFeatureLayer.SELECTION_METHOD.NEW, new CallbackListener<FeatureSet>() {
                    ...
                    public void onCallback(FeatureSet queryResults) {
                        ...
                        if (queryResults.getGraphics().length > 0) {
                            ...
                            listAdapter = new AttributeListAdapter(AddComponentActivity.this, selectedFeatureLayer.getFields(), selectedFeatureLayer.getTypes(),
        ...
        return new CallbackListener<FeatureEditResult[][]>() {
            ...
            public void onCallback(FeatureEditResult[][] result) {
                ...
                if (result[2] != null && result[2][0] != null && result[2][0].isSuccess()) {
                    ...
                    Log.d(AddComponentActivity.TAG, "Success updating feature with id=" + result[2][0].getObjectId());
            ...
            public void onError(Throwable e) {
                ...
                Log.d(AddComponentActivity.TAG, "error updating feature: " + e.getLocalizedMessage());
