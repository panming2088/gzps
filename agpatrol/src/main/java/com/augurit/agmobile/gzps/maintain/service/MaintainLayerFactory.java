...
public class MaintainLayerFactory extends PatrolLayerFactory {
    private Context mContext;

    private Set<MaintainBatchInfo.RowsBean> mBatchList;

    public MaintainLayerFactory(Set<MaintainBatchInfo.RowsBean> batch, Context context) {
        this.mBatchList = batch;
...
//                }
            ...
            if (layerInfo.getLayerName().contains(PatrolLayerPresenter.COMPONENT_LAYER)) {
                ...
                if (!ListUtil.isEmpty(childLayer) && !TextUtils.isEmpty(userOrg)) {
                    ...
                    HashMap<Integer, String> layerDefs = new HashMap<Integer, String>(16);
                    for (LayerInfo child : childLayer) {
                        if (child.getLayerName().contains("排水管")) {
                            layerDefs.put(child.getLayerId(), where);
                        }
                    }
                    ((ArcGISDynamicMapServiceLayer) layer).setLayerDefinitions(layerDefs);
    ...
    private String getWhere(Set<MaintainBatchInfo.RowsBean> batch) {
        if (batch == null || batch.size() == 0) return "";
        StringBuilder sb = new StringBuilder(" OBJECTID  in ( ");
        for (MaintainBatchInfo.RowsBean info : batch) {
