...
public class PublicAffairActivity extends BaseActivity implements IDrawerController, View.OnClickListener, Animation.AnimationListener {
    ...
    private FlexRadioGroup fbDistrict, fbUploadType, fbFacilityType, fbPipeType, fbPipeDirection;
     ...
     * 设施类型：窨井，雨水口等（日常巡检列表）
     */
    private String facilityType1 = null;
...
//            "南沙", "海珠", "荔湾", "花都", "越秀",
//            "增城", "从化", "净水公司"};
private String[] districtConditions = {"全部", "市排水公司", "科学城排水公司", "番禺污水公司", "南沙排水公司",
        "花都排水公司", "增城排水公司", "从化排水公司", "其他"};

    public static final String[] uploadTypeConditions = {"全部", "数据新增", "数据校核","数据删除"};

    private String[] facilityTypeConditions = {"全部", "窨井", "雨水口", "排水口","接驳井"};
    private String[] ssTypeConditions = {"全部", "窨井", "雨水口", "排水口"};
    private String[] pipeTypeConditions = {"全部", "雨污合流", "污水", "雨水"};
    private String[] pipedirection = {"全部", "正向", "反向"};
    ...
    private View ll_pipe_condition;
    private FlexRadioGroup fbFacilityType1;//日常巡检列表的设施类型
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                if (position == 0) {
                    ...
                    ll_facility_condition.findViewById(R.id.ll_type_layout).setVisibility(View.VISIBLE);
                    ll_facility_condition.findViewById(R.id.ll_ss_type).setVisibility(View.VISIBLE);
                    ll_facility_condition.findViewById(R.id.ll_ss_type_1).setVisibility(View.GONE);
                    ll_facility_condition.findViewById(R.id.ll_pipetype_layout).setVisibility(View.GONE);
                    ll_facility_condition.findViewById(R.id.ll_pipedirect_layout).setVisibility(View.GONE);
//                    iv_open_search.setVisibility(View.GONE);
                } else if (position == 1) {
                    ll_facility_condition.setVisibility(View.VISIBLE);
                    ll_facility_condition.findViewById(R.id.ll_type_layout).setVisibility(View.VISIBLE);
                    ll_facility_condition.findViewById(R.id.ll_ss_type).setVisibility(View.GONE);
                    ll_facility_condition.findViewById(R.id.ll_ss_type_1).setVisibility(View.GONE);
                    ll_facility_condition.findViewById(R.id.ll_pipetype_layout).setVisibility(View.VISIBLE);
                    ll_facility_condition.findViewById(R.id.ll_pipedirect_layout).setVisibility(View.GONE);
                    ll_event_condition.setVisibility(View.GONE);
//                    iv_open_search.setVisibility(View.VISIBLE);
                    ...
                    ll_facility_condition.findViewById(R.id.ll_type_layout).setVisibility(View.GONE);
                    ll_facility_condition.findViewById(R.id.ll_ss_type).setVisibility(View.VISIBLE);
                    ll_facility_condition.findViewById(R.id.ll_ss_type_1).setVisibility(View.GONE);
                    ll_facility_condition.findViewById(R.id.ll_ss_type_1).setVisibility(View.VISIBLE);
                    ll_facility_condition.findViewById(R.id.ll_pipetype_layout).setVisibility(View.GONE);
                    ll_facility_condition.findViewById(R.id.ll_pipedirect_layout).setVisibility(View.GONE);
                } else {
                    ll_facility_condition.setVisibility(View.VISIBLE);
        ...
        fbFacilityType1 = (FlexRadioGroup) findViewById(R.id.fbl_filter_structure_1);
        fbPipeType = (FlexRadioGroup) findViewById(R.id.fbl_filter_pipetype);
        fbPipeDirection = (FlexRadioGroup) findViewById(R.id.fbl_filter_pipedirect);
...
//        } else {
//            String parentOrg = facilityAffairService.getParentOrgOfCurrentUser();
//            String[] district = new String[]{StringUtil.isEmpty(mHashMap.get(parentOrg))?parentOrg:mHashMap.get(parentOrg)};
...
//        }
        ...
        createRadioButton(ssTypeConditions, fbFacilityType1);
        createRadioButton(pipeTypeConditions, fbPipeType);
        createRadioButton(pipedirection, fbPipeDirection);
        ...
        findViewById(R.id.tv_structure_1).setOnClickListener(this);
...
//        mHashMap.put("其他","其他");
//        mHashMap.put("黄埔","科学城排水公司");
//        mHashMap.put("番禺","番禺污水公司");
//        mHashMap.put("南沙","南沙排水公司");
//        mHashMap.put("花都","花都排水公司");
//        mHashMap.put("增城","增城排水公司");
//        mHashMap.put("从化","从化排水公司");
//    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_clear:
                ...
                fbFacilityType1.check(ssTypeConditions[0]);
                fbPipeType.check(pipeTypeConditions[0]);
                fbPipeDirection.check(pipedirection[0]);
                resetDate();
                break;
            case R.id.btn_submit:
                ...
                RadioButton rbStructure1 = (RadioButton) findViewById(fbFacilityType1.getCheckedRadioButtonId());
                if (rbStructure1 != null) {
                    facilityType1 = rbStructure1.getText().toString();
                }


                RadioButton rbPipeType = (RadioButton) findViewById(fbPipeType.getCheckedRadioButtonId());
                if (rbPipeType != null) {
                    pipeType = rbPipeType.getText().toString();
                }

                RadioButton rbPipeDirection = (RadioButton) findViewById(fbPipeDirection.getCheckedRadioButtonId());
                if (rbPipeDirection != null) {
                    direction = rbPipeDirection.getText().toString();
                }
                int currentItem = viewPager.getCurrentItem();

                if(currentItem == 0){
                    FacilityAffairFilterConditionEvent facilityAffairFilterConditionEvent =
                            new FacilityAffairFilterConditionEvent(StringUtil.isEmpty(mHashMap.get(district))?district:mHashMap.get(district), uploadType, facilityType, startDate, endDate);
                    EventBus.getDefault().post(facilityAffairFilterConditionEvent);
                }else if(currentItem == 3){
                    DayPatrolFilterConditionEvent dayPatrolFilterConditionEvent =
                            new DayPatrolFilterConditionEvent(StringUtil.isEmpty(mHashMap.get(district))?district:mHashMap.get(district), facilityType1, startDate, endDate);
                    EventBus.getDefault().post(dayPatrolFilterConditionEvent);
                }else{
                    PipeFilterConditionEvent pipeFilterConditionEvent = new PipeFilterConditionEvent(StringUtil.isEmpty(mHashMap.get(district))?district:mHashMap.get(district),pipeType,uploadType, startDate, endDate);
                    EventBus.getDefault().post(pipeFilterConditionEvent);
                }
            ...
            case R.id.tv_structure:
                ...
                break;
            case R.id.tv_structure_1:
                startAnim(fbFacilityType1, (TextView) v);
                break;
            case R.id.tv_date:
