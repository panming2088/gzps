...
//        String supportUrl = "http://192.168.43.139:8080/agsupport_swj/";
    ...
    public Observable<EventAffair> search(int pageNo, int pageSize, String district, String componentTypeCode, String eventTypeCode,Long startTime,Long endTime, String state,Long id) {
        User user = new LoginRouter(mContext, AMDatabase.getInstance()).getUser();
        return mEventApi.search(user.getLoginName(), pageNo, pageSize, null, district, componentTypeCode, eventTypeCode, state, startTime, endTime,id)
    ...
    public Observable<EventAffair> searchByObjectId(int pageNo, int pageSize, String objectId, String usid) {
        User user = new LoginRouter(mContext, AMDatabase.getInstance()).getUser();
        return mEventApi.searchByObjectId(user.getLoginName(), pageNo, pageSize, objectId, usid)
                .map(new Func1<Result2<EventAffair>, EventAffair>() {
                    @Override
                    public EventAffair call(Result2<EventAffair> responseBody) {
                        try {
    ...
    public Observable<EventAffair> search(int pageNo, int pageSize, String keyword) {
        User user = new LoginRouter(mContext, AMDatabase.getInstance()).getUser();
        return mEventApi.search(user.getLoginName(), pageNo, pageSize, keyword, null, null, null, null, null, null,null)
                .map(new Func1<Result2<EventAffair>, EventAffair>() {
                    @Override
                    public EventAffair call(Result2<EventAffair> responseBody) {
                        try {
     ...
     * @return
     */
    public Observable<List<Assigneers>> getTaskUserByloginName(String objectId) {
        User user = new LoginRouter(mContext, AMDatabase.getInstance()).getUser();
        return mEventApi.getTaskUserByloginName(user.getLoginName(),objectId);
    }
...
//                        String json = JsonUtil.getJson(responseBody);
    ...
    public void queryComponentsWithLayerName(final String usid, String objectId, final String layerName, final String url, final Callback2<Component> callback) {
        new ComponentService(mContext).queryComponentsWithLayerName(usid, objectId, layerName, url, callback);
    }
