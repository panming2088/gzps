...
public class FeedBackLayerFactory extends PatrolLayerFactory {
    private Context mContext;

    private Set<BatchInfo.Info> mBatchList;

    public FeedBackLayerFactory(Set<BatchInfo.Info> batch, Context context) {
...
//                }
    ...
    private String getWhere(Set<BatchInfo.Info> batch) {
        if (batch == null || batch.size() == 0) return "";
        StringBuilder sb = new StringBuilder(" QDBH in ( ");
        for (BatchInfo.Info info : batch) {
